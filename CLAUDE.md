...
# Run a single test
ctest --test-dir build -R TestAPILog --output-on-failure
...
src/internal/Common/            ← Shared utilities across all modules (cm prefix)
src/internal/Test/              ← Test allocation interception utilities (ts prefix)
...
| Common       | `cm`   | Shared result codes, error messages, utilities                       |
...
SceneManager → Common
Log → Common
Tests → public API + Internal headers + Test
```
...
**File layout:** Use the file-generation tool (`src/tools/GenScene/GenScene.c`) when creating new source files — it
generates the standard section-header boilerplate. Sections are separated by standardized comment blocks with 2 blank
lines before each header and 1 blank line after.

**Messages:** Check `CommonMessages.h` before adding new message macros. Message macros use prefixes `CSE_`, `CSQ_`.

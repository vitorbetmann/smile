 ...
 *
 * This utility function removes spaces, tabs, and newline characters from the
 * beginning and end of the input string `src`. It allocates a new string to
 * store the trimmed content, so the original string `src` remains unchanged.
 *
 * @param src The input string to be trimmed. If `NULL`, the function returns `NULL`.
 * @return A newly allocated string containing the trimmed content, or `NULL` if
 *         `src` is `NULL` or if memory allocation fails. The caller is responsible
 *         for freeing the returned string using `tsInternalFree()`. If the input
 *         string contains only whitespace or is empty after trimming, an empty
 *         string `""` will be returned (if memory allocation is successful).
 * @author Vitor Betmann
 */

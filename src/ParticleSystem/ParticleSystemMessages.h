#pragma once

// Module Name —————————————————————————————————————————————————————————————————————————————————————

#define ORI "ParticleSystem"

// Context Messages ————————————————————————————————————————————————————————————————————————————————

static const char MSG_INVALID_SPREAD_RANGE[] = "Outer spread must be greater than inner spread";
static const char MSG_INVALID_LIFETIME_RANGE[] = "min lifetime must not exceed max lifetime";
static const char MSG_INVALID_VELOCITY_RANGE[] = "min velocity must not exceed max velocity";
static const char MSG_INVALID_ACCELERATION_RANGE[] =
    "min acceleration must not exceed max acceleration";
